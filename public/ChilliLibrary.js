...
chilliController.urlRoot = function() {
	...
	var urlRoot =
		protocol +
		'://' +
		chilliController.host +
		(chilliController.port ? ':' + chilliController.port.toString() : '') +
		'/json/';
	return urlRoot;
};
...
chilliController.logonStep2 = function(resp) {
	...
	if (chilliController.uamService) {
		/* MD5 CHAP will be calculated by uamService */
...
chilliController.processReply = function(resp) {
	...
	/* Update clientState */
	if (
		resp.clientState === chilliController.stateCodes.NOT_AUTH ||
		resp.clientState === chilliController.stateCodes.AUTH ||
